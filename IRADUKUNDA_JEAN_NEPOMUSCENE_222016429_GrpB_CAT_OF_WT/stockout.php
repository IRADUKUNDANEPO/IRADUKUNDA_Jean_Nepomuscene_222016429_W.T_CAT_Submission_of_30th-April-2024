...
<!--stockout form using for insertion data to database-->

 <h1><u>StockOut Form</u></h1>
    <form method="post" onsubmit="return confirmInsert();">
        <label for="stockout_id">StockOut Id:</label>
        <input type="number" id="stockout_id" name="stockout_id"><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Convert date format if necessary (assumed already in correct format)
    $transdate = date('Y-m-d', strtotime($transdate)); // Adjust if needed

    // Prepare and bind the parameters
    $stmt = $connection->prepare("INSERT INTO Stock_out (StockOut_Id, Stock_Transaction_Id, Product_Id, Customer_Id, Transaction_date, Quantity) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("iiissi", $stockout_id, $trans_id, $pid, $custid, $transdate, $quantity);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of Products Stock_out</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Stock_out</h2></center>
    <table border="5">
        <tr>
          <!--StockOut_Id Stock_Transaction_Id  Product_Id  Customer_Id Transaction_date  Quantity-->
            <th>StockOut Id</th>
      ...
      include('Database_Connection.php');
        ...
        // Prepare SQL query to retrieve all stockout
        $sql = "SELECT * FROM Stock_out";
        $result = $connection->query($sql);

        // Check if there are any stockout
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $stockout = $row['StockOut_Id']; // Fetch the Product_Id
                echo "<tr>
                    <td>" . $row['StockOut_Id'] . "</td>
                    <td>" . $row['Stock_Transaction_Id'] . "</td>
                    <td>" . $row['Product_Id'] . "</td>
                    <td>" . $row['Customer_Id'] . "</td>
                    <td>" . $row['Transaction_date'] . "</td>
                    <td>" . $row['Quantity'] . "</td>
                    <td><a style='padding:4px' href='delete_stockout.php?StockOut_Id=$stockout'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_stockout.php?StockOut_Id=$stockout'>Update</a></td> 
                </tr>";
            }
        } else {
